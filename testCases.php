		...
		try {
			...
			if ($expectedException == false) {
				if ($actual != $expectedOutput) {
					throw new \Exception ("wrong output [$actual] for [$inputArray[0],$inputArray[1],$inputArray[2]]should be [$expectedOutput] </br>");
					$errors++;
				}
			} else {
				throw new \Exception ("wrong output [$actual] for [$inputArray[0],$inputArray[1],$inputArray[2]]should be [Exception]  </br>");
				$errors++;
			}
		} catch (\Exception $e) { 
			
			if ($expectedException == false) {
				throw new \Exception ("wrong output [Exception] for [$inputArray[0],$inputArray[1],$inputArray[2]]should be [$expectedOutput] </br>");
				$errors++;
			} else {
				//OK
			}
